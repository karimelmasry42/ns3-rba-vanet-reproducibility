...
void
AppendResultsCsv(const ScenarioConfig& config, const AggregateStats& stats)
{
    ...
    if (writeHeader)
    {
        output << "seed,run,num_vehicles,num_rsus,total_nodes,road_length_m,lane_count,lane_spacing_m,"
               << "coverage_range_m,tx_power_dbm,packet_size_bytes,beacon_interval_ms,warmup_s,"
               << "active_s,cleanup_s,phy_mode,prover_processing_delay_ms,"
               << "verifier_processing_delay_ms,"
    ...
    output << config.seed << ',' << config.run << ',' << config.numVehicles << ',' << config.numRsus
           ...
           << config.phyMode << ',' << config.proverProcessingDelayMs << ','
